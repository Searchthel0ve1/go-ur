...
.PHONY: gur gur-cross evm all test clean
...
gur-android:
	build/env.sh go run build/ci.go xgo -- --go=$(GO) --dest=$(GOBIN) --targets=android-21/aar -v ./cmd/gur
	@echo "Android cross compilation done:"
	@ls -ld $(GOBIN)/gur-android-*

gur-ios:
	build/env.sh go run build/ci.go xgo -- --go=$(GO) --dest=$(GOBIN) --targets=ios-7.0/framework -v ./cmd/gur
